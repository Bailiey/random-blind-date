...
const departmentData = {
    ...
    '국제학부': ['국제학부'],
    '농업생명과학대학': ['농업경제학과', '동물바이오시스템과학과', '동물자원생명과학과', '산림환경자연학과', '생물환경화학과', '스마트농업시스템기계공학과', '식물자원학과', '식품공학과', '원예학과', '응용생물학과', '지역환경토목학과', '환경소재공학과'],
...
function startParticipationFlow(matchType) {
    currentMatchType = matchType;
    const popup = document.getElementById('paymentPopupOverlay');
    if (popup) popup.style.display = 'flex';
}

// 참가비 팝업 확인 후 학생증 팝업 표시
function showStudentIdPopup() {
    closePopup('paymentPopupOverlay');
    const popup = document.getElementById('studentIdPopupOverlay');
    if (popup) popup.style.display = 'flex';
}

// 학생증 팝업 확인 후 개인정보 동의 페이지로 이동
function proceedToConsent() {
    closePopup('studentIdPopupOverlay');
    showPage(2);
}
...
// --- 4/5. 폼 제출 및 데이터 저장 기능 (로컬 스토리지) ---
/** ✨ 수정된 함수: 최종 제출 전 확인 팝업을 띄웁니다. */
function submitForm(type, event) {
    ...
    let htmlContent = ''; // 팝업에 표시될 HTML 내용
    ...
    if (type === '1on1') {
        participantData = {
            ...
            '참가자_학과': document.getElementById('dept1').value,
...
/** ✨ 새로 추가된 함수: 최종 확인 후 데이터를 저장하고 완료 페이지로 이동합니다. */
...
function filterNonNumeric(event) {
    // 복권 번호 및 전화번호 등에 사용: 숫자만 허용
    event.target.value = event.target.value.replace(/[^0-9]/g, '');
}

// 학번에 사용. 숫자(0-9)와 대문자 영어(A-Z)만 허용.
function filterStudentId(event) {
    // 소문자는 대문자로 변환한 후, 숫자(0-9)와 대문자(A-Z)가 아닌 모든 문자를 제거합니다.
    let value = event.target.value.toUpperCase();
    event.target.value = value.replace(/[^0-9A-Z]/g, '');
}